...
namespace Generation.Level
{
    public class GenProShaper : MonoBehaviour
    {
        ...
        {
            //builds every room
            for (int i = 0; i < GenProPlanner.instance.roomBuffer.Count; i++) CreateRoom(i);
        }

        void CreateRoom(int index)
        {
            //uses center and size to build a room with a round or more squared shape
            GenProPlanner.RoomGenerationInfo info = GenProPlanner.instance.roomBuffer[index];
        ...
        void AddBridges()
        {
            ...
            {
                Vector2Int roomCenter = GenProPlanner.instance.roomBuffer[i].centerPosition;
                Vector2Int nextRoomCenter = GenProPlanner.instance.roomBuffer[i + 1].centerPosition;
