...
public class Enemy_old : MonoBehaviour
{
    ...
    IEnumerator ResetPoisonCounter()
    {
        ...
        GameManager_old.instance.DealDamageToEnemy(ObjectsManager_old.instance.gameVariables.poisonDamage, this, false);
        yield return new WaitForSeconds(ObjectsManager_old.instance.gameVariables.poisonCooldown);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("PlayerAttack"))
        {
            ...
            {
                //receives damage
                GameManager_old.instance.DealDamageToEnemy(other.GetComponent<ObjectDamage>().damage, this);
                bleedGen.PlayRandomSound(0);
                stabGen.PlayRandomSound(0);
                _takenDamageSinceTimer = 0;
            }
            _rb.AddForce((_player.transform.position - transform.position).normalized * -PlayerAttacks_old.instance.bumpForce, ForceMode.Impulse);
            stunCounter = 1;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            //gets poisoned
            _poisonCounter = ObjectsManager_old.instance.gameVariables.poisonLenght;
        }
    }
    ...
    public void Death()
    {
        ...
        {
            Instantiate(ObjectsManager_old.instance.eyeToken, new Vector3(transform.position.x, _player.transform.position.y, transform.position.z), Quaternion.identity);
        }
        Destroy(gameObject);
    }
   ...
   private void HardcoreModeRegeneration()
   {
       ...
       if (GameManager_old.instance.hardcoreMode || isFromStela)
       {
