...
public class Chest_old : MonoBehaviour
{
    ...
    {            
        ...
        GameManager_old.instance.cmShake.ShakeCamera(7, 0.1f);
        canOpen = true;
    }
    ...
    IEnumerator ChestLoot()
    {
        ...
        if (randLoot <= 15)
        {
            GameObject item = Instantiate(ObjectsManager_old.instance.objectTemplate, transform.position + Vector3.up * 2, Quaternion.identity);
            //checks which items are already equipped and remove them from the possible items
            item.GetComponent<Item_old>().objectID = ObjectsManager_old.instance.itemList[Random.Range(0, ObjectsManager_old.instance.itemList.Count)];
            item.GetComponent<Item_old>().itemCost = 0;
            item.SetActive(true);
        ...
        {
            //heal,eyes
            GameObject heal = Instantiate(ObjectsManager_old.instance.healItem, lootDropPos + Vector3.left,
                quaternion.identity);
            heal.GetComponent<Item_old>().itemCost = 0;
            heal.GetComponent<Item_old>().isDuplicate = false;
            heal.GetComponent<Item_old>().isFromAShop = false;
            heal.GetComponent<Item_old>().costText.text = "0";
            heal.SetActive(true);
            yield return new WaitForSeconds(0.5f);
            GameObject eyes = Instantiate(ObjectsManager_old.instance.eyeToken, lootDropPos + Vector3.right,
                quaternion.identity);
            eyes.SetActive(true);
        }
