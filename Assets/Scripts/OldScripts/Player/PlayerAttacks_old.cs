...
public class PlayerAttacks_old : MonoBehaviour
{
    public static PlayerAttacks_old instance;
    ...
    private void Start() => _pc = PlayerController__old.instance;
    private void Update()
    {
    ...
    IEnumerator AttackCoroutine()
    {
        ...
        if (_pc.currentState != PlayerController__old.PlayerStates.Dash)
        {
            _rb.velocity = Vector3.zero;
        }
        _pc.SwitchState(PlayerController__old.PlayerStates.Attack);
        ...
        comboState = ComboState.Default;
        _pc.SwitchState(PlayerController__old.PlayerStates.Run);
        _pc.canMove = true;
        isAttacking = false;
    }
    ...
    IEnumerator DelayRockShake(int power, float delay)
    {
        ...
        GameManager_old.instance.cmShake.ShakeCamera(power, 0.1f);
    }
    #endregion
    ...
    IEnumerator SmashCoroutine()
    {
        ...
        _pc.SwitchState(PlayerController__old.PlayerStates.Attack); //stops walking
        _pc.canMove = false;
        ...
        if (ObjectsManager_old.instance.stinkyFish)
        {
            ObjectsManager_old.instance.PlayActivationVfx(2);
            switch (smashPower)
            {
                case 0 : 
        ...
        if (ObjectsManager_old.instance.earthQuake)
        {
            ObjectsManager_old.instance.PlayActivationVfx(5);
            switch (smashPower)
            {
                case 0 : 
        ...
        #endregion
        StartCoroutine(GameManager_old.instance.ShakeCam(shakeStrengh));
        yield return new WaitForSeconds(activeLength);
        //------------recovery state
    ...
    private void OnSmashHold()
    {
        ...
        {
            _pc.currentState = PlayerController__old.PlayerStates.Attack;
            _rb.velocity = Vector3.zero;
    ...
    int GetAttackAnimation(Vector3 playerDir)
    {
        ...
        PlayerController__old.instance.spriteRenderer.flipX = xVal < 0;
    ...
    {
        if (currentAttackState is AttackState.Default or AttackState.Recovery && PlayerController__old.instance.currentState != PlayerController__old.PlayerStates.Dash)
        {
            _pc.canMove = true;
            StopAllCoroutines();
