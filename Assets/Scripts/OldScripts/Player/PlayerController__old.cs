...
public class PlayerController__old : MonoBehaviour
{
    public static PlayerController__old instance;
    ...
    void FixedUpdate()
    {
        if (PlayerAttacks_old.instance.currentAttackState == PlayerAttacks_old.AttackState.Default)
        {
    ...
    {
        if ((currentState == PlayerStates.Run || PlayerAttacks_old.instance.currentAttackState == PlayerAttacks_old.AttackState.Recovery) && _dashesAvailable > 0 && dashCooldownTimer <= 0)
        {
            _dashesAvailable--;
            StartCoroutine(DashSequence());
        }
        else if (ObjectsManager_old.instance.killingSpreeTimer > 0) //uses killing spree timer
        {
            ObjectsManager_old.instance.killingSpreeTimer = 0;
            StartCoroutine(DashSequence());
        }
    }
    IEnumerator DashSequence()
    {
        PlayerAttacks_old.instance.InterruptAttack();
        SwitchState(PlayerStates.Dash);
        isDashing = true;
        //plays fx and sfx
        PlayerAttacks_old.instance.burnVfxPulling.PlaceDashFx();
        dashTrail.Play();
    ...
    void MovingAnimations()
    {
        ...
        {
            //idle
            if (PlayerAttacks_old.instance.currentAttackState != PlayerAttacks_old.AttackState.Active 
                        && PlayerAttacks_old.instance.currentAttackState != PlayerAttacks_old.AttackState.Startup 
                        && PlayerAttacks_old.instance.currentAttackState != PlayerAttacks_old.AttackState.Recovery
